...
namespace sframe {
  ...
  std::ostream& operator<<(std::ostream& str, const bytes& data) {
  ...
  static auto evp_cipher_ctx_free = [](EVP_CIPHER_CTX* ptr) { EVP_CIPHER_CTX_free(ptr); };

  using scoped_evp_ctx = std::unique_ptr<EVP_CIPHER_CTX, decltype(evp_cipher_ctx_free)>;

  static std::runtime_error openssl_error() {
    auto code = ERR_get_error();
    return std::runtime_error(ERR_error_string(code, nullptr));
  }

  static const EVP_CIPHER* openssl_cipher(CipherSuite suite) {
    switch (suite) {
  ...
  static int openssl_tag_size(CipherSuite suite) {
    switch (suite) {
      case CipherSuite::AES_GCM_128:
      case CipherSuite::AES_GCM_256:
        return 16;
  ...
  static size_t openssl_nonce_size(CipherSuite suite) {
    switch (suite) {
      case CipherSuite::AES_GCM_128:
      case CipherSuite::AES_GCM_256:
        return 12;
  ...
  static size_t seal(CipherSuite suite, const bytes& key, const bytes& nonce, size_t aad_size,
                     uint8_t* ct, size_t ct_size, const uint8_t* pt, size_t pt_size) {
    ...
    if (1 != EVP_EncryptUpdate(ctx.get(), ct + aad_size, &outlen, pt, pt_size_int)) {
      throw openssl_error();
    }
    ...
    if (1 != EVP_CIPHER_CTX_ctrl(ctx.get(), EVP_CTRL_GCM_GET_TAG, tag_size, tag_ptr)) {
      throw openssl_error();
    }
  ...
  static size_t open(CipherSuite suite, const bytes& key, const bytes& nonce, size_t aad_size,
                     uint8_t* pt, size_t pt_size, const uint8_t* ct, size_t ct_size) {
    auto tag_size = openssl_tag_size(suite);
    ...
    auto aad_size_int = static_cast<int>(aad_size);
    if (aad_size > 0) {
      if (1 != EVP_DecryptUpdate(ctx.get(), nullptr, &out_size, aad_start, aad_size_int)) {
        throw openssl_error();
      }
    }

    auto inner_ct_size_int = static_cast<int>(inner_ct_size);
    if (1 != EVP_DecryptUpdate(ctx.get(), pt, &out_size, ct_start, inner_ct_size_int)) {
      throw openssl_error();
    }
  ...
  Context::Context(CipherSuite suite_in) : suite(suite_in) {}

  void Context::add_key(KeyID key_id, bytes key) {
    state.insert_or_assign(key_id, KeyState{std::move(key), 0});
  }

  static size_t encode_uint(uint64_t val, uint8_t* start) {
    size_t size = 1;
    while (val >> (8 * size) > 0) {
      size += 1;
    }
  ...
  static uint64_t decode_uint(const uint8_t* data, size_t size) {
    uint64_t val = 0;
    for (size_t i = 0; i < size; i++) {
  ...
  static bytes form_nonce(CipherSuite suite, Counter ctr) {
  ...
  static size_t encode_header(KeyID kid, Counter ctr, uint8_t* data) {
    size_t kid_size = 0;
    if (kid > 0x07) {
  ...
  static std::tuple<KeyID, Counter, size_t> decode_header(const uint8_t* data, size_t size) {
  ...
  bytes Context::protect(KeyID kid, const bytes& plaintext) {
    ...
    seal(suite, key, nonce, hdr_size, ct.data(), ct.size(), plaintext.data(), plaintext.size());
    return ct;
  }

  bytes Context::unprotect(const bytes& ciphertext) {
    auto [kid, ctr, hdr_size] = decode_header(ciphertext.data(), ciphertext.size());
    auto tag_size = openssl_tag_size(suite);
    ...
    open(suite, key, nonce, hdr_size, pt.data(), pt.size(), ciphertext.data(), ciphertext.size());
    return pt;
  }
