...
struct HeaderTestVector
{
  ...
  {
    // Decode
    const auto decoded = UNWRAP(Header::parse(encoded));
...
struct AesCtrHmacTestVector
{
  ...
  void verify() const
  {
    ...
    const auto ct_out =
      UNWRAP(seal(cipher_suite, key, nonce, ciphertext, aad, pt));
    REQUIRE(ct_out == ct);

    // Open
    auto plaintext = bytes(pt.data.size());
    const auto pt_out =
      UNWRAP(open(cipher_suite, key, nonce, plaintext, aad, ct));
    REQUIRE(pt_out == pt);
  }
};
...
struct SFrameTestVector
{
  ...
  void verify() const
  {
    ...
    UNWRAP(send_ctx.add_key(kid, KeyUsage::protect, base_key));

    auto ct_data = owned_bytes<128>();
    auto next_ctr = uint64_t(0);
    while (next_ctr < ctr) {
      UNWRAP(send_ctx.protect(kid, ct_data, pt, metadata));
      next_ctr += 1;
    }

    const auto ct_out = UNWRAP(send_ctx.protect(kid, ct_data, pt, metadata));
    ...
    UNWRAP(recv_ctx.add_key(kid, KeyUsage::unprotect, base_key));

    auto pt_data = owned_bytes<128>();
    auto pt_out = UNWRAP(recv_ctx.unprotect(pt_data, ct, metadata));
    REQUIRE(pt_out == pt);
  }
};
